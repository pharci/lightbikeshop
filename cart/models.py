...
class Cart(models.Model):
    ...
    def _compute_discount(self, subtotal: Decimal) -> Decimal:
        """Посчитать скидку от применённого к корзине промокода (если применим сейчас)."""
        pc = self.promo_code
        if not pc:
            return Decimal("0")
        ...
        if not ok:
            return Decimal("0")
        return Decimal(str(pc.calculate_discount(subtotal))).quantize(Decimal("1."))
    
    def _get_promo_obj(self):
    ...
    def get_discount(self) -> Decimal:
        ...
        return max(Decimal("0"), discount).quantize(Decimal("1."))
    ...
    def get_cart_total_price(self) -> Decimal:
        ...
        return max(Decimal("0"), total).quantize(Decimal("1."))
...
class SessionCart:
    ...
    def _compute_discount(self, subtotal: Decimal) -> Decimal:
        pc = self._get_promo_obj()
        if not pc:
            return Decimal("0")
        user = getattr(self.session, "user", None)  # не всегда есть; валидируем без него
        ok, _ = pc.can_apply(user=None, subtotal=subtotal)  # на сессии польз-ль может быть аноним
        if not ok:
            return Decimal("0")
        return Decimal(str(pc.calculate_discount(subtotal))).quantize(Decimal("1."))
...
class Order(models.Model):
    ...
    owner_token = models.UUIDField(
        "Токен владельца", default=uuid.uuid4,
    ...
    amount         = models.DecimalField("Сумма заказа", max_digits=10, decimal_places=2, default=0)
    ...
    payment_id     = models.CharField('ID платежа', max_length=64, blank=True)
    ...
    promo_discount_amount = models.DecimalField(
        "Скидка по промокоду", max_digits=10, decimal_places=2, default=0
    )
    ...
    # Сумма по строкам (без скидок)
    def get_subtotal_price(self) -> Decimal:
        agg = self.items.aggregate(total=Sum(F('price') * F('quantity')))
        return (agg['total'] or Decimal("0")).quantize(Decimal("1."))

    # Итог с учётом скидки промо
    def get_total_price(self) -> Decimal:
        base = self.get_subtotal_price()
        disc = Decimal(self.promo_discount_amount or 0)
        return max(Decimal("0"), (base - disc)).quantize(Decimal("1."))
